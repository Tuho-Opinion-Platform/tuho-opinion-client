...
//     return votes[opinionId] !== undefined;
//   });
...
//   useEffect(() => {
//     localStorage.setItem(`countUp-${opinionId}`, countUp);
//     localStorage.setItem(`countDown-${opinionId}`, countDown);
//     // Update the votes record in localStorage
//     const votes = JSON.parse(localStorage.getItem(`votes-${user?._id}`) || "{}");
//     if (hasVoted) {
//       votes[opinionId] = true;
//       localStorage.setItem(`votes-${user?._id}`, JSON.stringify(votes));
//     }
//   }, [countUp, countDown, opinionId, user?._id, hasVoted]);
...
//               <button onClick={increment}><p>Agree {countUp}</p></button>
//               <button onClick={decrement}>Disagree {countDown}</button>
//             </div>
